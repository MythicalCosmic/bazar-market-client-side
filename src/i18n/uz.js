export default {
  // Navigation
  'nav.home': 'Bosh sahifa',
  'nav.categories': 'Kategoriyalar',
  'nav.orders': 'Buyurtmalar',
  'nav.profile': 'Profil',

  // Bottom nav short labels (single-word, fit narrow phones)
  'nav.short.home': 'Bosh',
  'nav.short.categories': 'Katalog',
  'nav.short.favorites': 'Sevimli',
  'nav.short.orders': 'Buyurtma',
  'nav.short.profile': 'Profil',

  // Header
  'header.current_location': 'Hozirgi manzil',
  'header.no_address': 'Manzilni tanlang',
  'header.search_placeholder': 'Qidirish...',

  // Search
  'search.recent': 'So\'nggi qidiruvlar',
  'search.clear_all': 'Tozalash',
  'search.no_results': 'Hech narsa topilmadi',
  'search.try_different': 'Boshqa so\'z bilan urinib ko\'ring',
  'search.hero_title': 'Nimani qidiryapsiz?',
  'search.hero_subtitle': 'Mahsulotni nomi yoki kategoriyasi bo\'yicha qidiring',
  'search.results_count': '{count} ta natija',

  // Home
  'home.featured': 'Ommabop mahsulotlar',
  'home.see_all': 'Hammasi',
  'home.for_you': 'Siz uchun',
  'home.categories': 'Kategoriyalar',

  // Categories
  'cat.all': 'Yangiliklar',
  'cat.hot': 'Ommabop',
  'cat.dairy': 'Sut mahsulotlari',
  'cat.bread': 'Non va pishiriqlar',
  'cat.sweets': 'Shirinliklar',
  'cat.fruits': 'Mevalar',
  'cat.drinks': 'Ichimliklar',
  'cat.snacks': 'Gazaklar',
  'cat.hygiene': 'Gigiena',
  'categories.subcategories': 'turkum',
  'categories.all_in': 'Hammasi:',
  'categories.no_products': 'Mahsulotlar topilmadi',
  'categories.search_placeholder': 'Kategoriya qidirish...',
  'categories.show_all': 'Hammasini ko\'rish',

  // Product badges
  'badge.new': 'YANGI',
  'badge.hot': 'HIT',

  // Cart
  'cart.title': 'Savatcha',
  'cart.items_count': 'Mahsulotlar',
  'cart.empty_title': 'Savatcha bo\'sh',
  'cart.empty_subtitle': 'Katalogdan mahsulot qo\'shing',
  'cart.go_catalog': 'Katalogga',
  'cart.add_more': 'Buyurtmaga qo\'shish',
  'cart.add': 'Qo\'shish',
  'cart.your_order': 'Sizning buyurtmangiz',
  'cart.products': 'Mahsulotlar',
  'cart.discount': 'Chegirma',
  'cart.delivery': 'Yetkazib berish',
  'cart.total': 'Jami',
  'cart.checkout': 'Buyurtma berish',
  'cart.clear_confirm': 'Savatchani tozalaysizmi?',
  'cart.clear_subtitle': 'Barcha mahsulotlar o\'chiriladi',
  'cart.clear_yes': 'Ha, tozalash',
  'cart.promo_code': 'Promo kod',
  'cart.promo_applied': 'Promo kod qo\'llanildi!',

  // Checkout
  'checkout.title': 'Buyurtma',
  'checkout.home': 'Uy',
  'checkout.phone': 'Telefon',
  'checkout.delivery_time': 'Yetkazib berish vaqti',
  'checkout.delivery_minutes': '25-30 daqiqa',
  'checkout.payment': 'To\'lov usuli',
  'checkout.cash': 'Naqd pul',
  'checkout.total': 'Jami',
  'checkout.place_order': 'Buyurtma berish',
  'checkout.detecting_location': 'Joylashuv aniqlanmoqda...',
  'checkout.detecting_address': 'Manzil aniqlanmoqda...',
  'checkout.address_unknown': 'Manzil aniqlanmadi',
  'checkout.location_detected': 'Joylashuv aniqlandi',
  'checkout.location_default': 'Toshkent (standart)',
  'checkout.location_unavailable': 'Geolokatsiya mavjud emas',
  'checkout.change': 'O\'zgartirish',
  'checkout.you_are_here': 'Siz shu yerdasiz',
  'checkout.map_unavailable': 'Xarita yuklanmadi',

  // Orders
  'orders.title': 'Mening buyurtmalarim',
  'orders.empty_title': 'Buyurtmalar yo\'q',
  'orders.empty_subtitle': 'Sizning buyurtmalaringiz shu yerda ko\'rinadi',
  'orders.total': 'Jami',
  'orders.repeat': 'Takrorlash',
  'orders.track': 'Kuzatish',
  'orders.expected_arrival': 'Kutilayotgan yetib kelish',
  'orders.arriving_in': 'Taxminan {min} daqiqada',
  'orders.status.accepted': 'Qabul qilindi',
  'orders.status.preparing': 'Tayyorlanmoqda',
  'orders.status.on_way': 'Yo\'lda',
  'orders.status.delivered': 'Yetkazildi',
  'orders.status_label.preparing': 'Tayyorlanmoqda',
  'orders.status_label.delivering': 'Yo\'lda',
  'orders.status_label.delivered': 'Yetkazildi',

  // Profile
  'profile.username': 'Foydalanuvchi',
  'profile.edit': 'Tahrirlash',
  'profile.orders_count': 'Buyurtmalar',
  'profile.favorites_count': 'Sevimlilar',
  'profile.coupons_count': 'Kuponlar',
  'profile.my_orders': 'Mening buyurtmalarim',
  'profile.categories': 'Kategoriyalar',
  'profile.my_coupons': 'Kuponlarim',
  'profile.payment_methods': 'To\'lov usullari',
  'profile.help': 'Yordam va qo\'llab-quvvatlash',
  'profile.logout': 'Chiqish',
  'profile.language': 'Til',
  'profile.theme': 'Mavzu',
  'profile.theme_light': 'Yorug\'',
  'profile.theme_dark': 'Qorong\'u',
  'profile.theme_auto': 'Avto',
  'profile.settings': 'Sozlamalar',
  'profile.save': 'Saqlash',
  'profile.cancel': 'Bekor qilish',
  'profile.name': 'Ism',
  'profile.phone': 'Telefon',

  'profile.not_verified': 'Tasdiqlanmagan',

  // Checkout (extra)
  'checkout.card': 'Karta orqali',

  // Login
  'login.welcome': 'Xush kelibsiz!',
  'login.title': 'Kirish',
  'login.subtitle': 'Hisobingizga kiring',
  'login.phone_subtitle': 'Telefon raqamingizni kiriting — sizga SMS-kod yuboramiz',
  'login.send_code': 'Kod yuborish',
  'login.use_different_phone': 'Boshqa raqam ishlatish',
  'login.rate_limited': '{sec} soniyadan keyin urinib ko\'ring',
  'login.button': 'Kirish',
  'login.or': 'yoki',
  'login.no_account': 'Hisobingiz yo\'qmi? Ro\'yxatdan o\'ting',
  'login.have_account': 'Hisobingiz bormi? Kiring',

  // Register
  'register.title': 'Ro\'yxatdan o\'tish',
  'register.subtitle': 'Buyurtma berish uchun ro\'yxatdan o\'ting',
  'register.first_name': 'Ism',
  'register.last_name': 'Familiya',
  'register.first_name_placeholder': 'Ismingizni kiriting',
  'register.last_name_placeholder': 'Familiyangizni kiriting',
  'register.phone': 'Telefon raqam',
  'register.send_code': 'Kod yuborish',
  'register.skip': 'Keyinroq',
  'register.verify_title': 'Tasdiqlash',
  'register.verify_subtitle': 'Telefoningizga yuborilgan kodni kiriting',
  'register.code_sent_to': 'Kod yuborildi:',
  'register.resend': 'Qayta yuborish',
  'register.verify': 'Tasdiqlash',
  'register.fill_all': 'Barcha maydonlarni to\'ldiring',
  'register.invalid_phone': 'Telefon raqam noto\'g\'ri',
  'register.enter_code': '6 xonali kodni kiriting',
  'register.wrong_code': 'Kod noto\'g\'ri',
  'register.optional': '(ixtiyoriy)',

  // Settings
  'settings.language_desc': 'Ilova tilini tanlang',
  'settings.theme_desc': 'Yorug\' yoki qorong\'u rejim',
  'settings.notifications': 'Bildirishnomalar',
  'settings.notifications_desc': 'Buyurtma yangilanishlari',
  'settings.logout_all': 'Barcha qurilmalardan chiqish',
  'settings.logout_all_desc': 'Boshqa qurilmalardagi sessiyalarni yakunlash',
  'settings.logout_all_confirm_title': 'Barcha qurilmalardan chiqish?',
  'settings.logout_all_confirm_subtitle': 'Siz barcha qurilmalardan chiqib ketasiz',
  'settings.logout_all_yes': 'Ha, chiqish',
  'settings.logout_all_done': 'Barcha sessiyalar yakunlandi',
  'settings.delete_account': 'Hisobni o\'chirish',
  'settings.delete_account_desc': 'Bu amalni qaytarib bo\'lmaydi',
  'settings.delete_confirm_title': 'Hisobni butunlay o\'chirasizmi?',
  'settings.delete_confirm_subtitle': 'Barcha ma\'lumotlaringiz yo\'qoladi va tiklab bo\'lmaydi.',
  'settings.delete_yes': 'Ha, o\'chirish',
  'settings.account_deleted': 'Hisob o\'chirildi',

  // Support
  'support.title': 'Yordam markazi',
  'support.hero_title': 'Sizga qanday yordam bera olamiz?',
  'support.hero_subtitle': 'Biz har doim yordam berishga tayyormiz',
  'support.contact_us': 'Biz bilan bog\'lanish',
  'support.call': 'Qo\'ng\'iroq qilish',
  'support.telegram': 'Telegram',
  'support.email': 'Elektron pochta',
  'support.faq_title': 'Ko\'p so\'raladigan savollar',
  'support.faq1_q': 'Buyurtmani qanday berishim mumkin?',
  'support.faq1_a': 'Mahsulotlarni savatchaga qo\'shing va buyurtma berish tugmasini bosing. Manzilni tanlang va to\'lov usulini belgilang.',
  'support.faq2_q': 'Yetkazib berish qancha vaqt oladi?',
  'support.faq2_a': 'Odatda 25-30 daqiqa ichida yetkazib beramiz. Buyurtma holati sahifasida aniq vaqtni kuzatishingiz mumkin.',
  'support.faq3_q': 'Buyurtmani bekor qilish mumkinmi?',
  'support.faq3_a': 'Ha, buyurtma tayyorlanishdan oldin bekor qilishingiz mumkin. Buyurtmalar sahifasida bekor qilish tugmasini bosing.',
  'support.faq4_q': 'Qaysi to\'lov usullari mavjud?',
  'support.faq4_a': 'Hozirda karta orqali to\'lov va naqd pul bilan to\'lov qabul qilamiz.',
  'support.working_hours': 'Ish vaqti',
  'support.every_day': 'Har kuni',

  // Store open / closed
  'store.open': 'Ochiq',
  'store.closed': 'Yopiq',
  'checkout.store_closed_title': 'Do\'kon yopiq',
  'checkout.store_closed': 'Hozir yopiqmiz. Buyurtmalar {open} dan {close} gacha qabul qilinadi.',
  'checkout.store_closed_btn': 'Hozir yopiq',

  // Orders — customer support
  'orders.support_title': 'Yordam kerakmi?',
  'orders.support_subtitle': 'Savollar, ko\'p so\'raladigan savollar va aloqa',
  'orders.help_center': 'Yordam markazi',
  'orders.help_center_sub': 'Savol-javoblar va ko\'p so\'raladigan savollar',
  'orders.delivery_phone': 'Yetkazib berish xizmati',

  // Favorites
  'favorites.title': 'Sevimlilar',
  'favorites.empty_title': 'Sevimlilar bo\'sh',
  'favorites.empty_subtitle': 'Yoqtirgan mahsulotlarni belgilang',

  // Payment methods
  'payment.card_title': 'Karta orqali',
  'payment.card_desc': 'Visa, Mastercard, Humo, UzCard',
  'payment.cash_title': 'Naqd pul',
  'payment.cash_desc': 'Yetkazib berishda to\'lash',
  'payment.info': 'To\'lov usulini buyurtma berish paytida ham o\'zgartirishingiz mumkin.',

  // Logout
  'logout.title': 'Chiqishni xohlaysizmi?',
  'logout.subtitle': 'Hisobingizdan chiqasiz',
  'logout.confirm': 'Ha, chiqish',

  'profile.verify_to_order': 'Buyurtma berish uchun ro\'yxatdan o\'ting',

  // Nav extras
  'nav.favorites': 'Sevimlilar',

  // Addresses
  'addresses.title': 'Manzillar',
  'addresses.add_new': 'Yangi manzil',
  'addresses.label_placeholder': 'Nomi (masalan: Uy, Ish)',
  'addresses.address_placeholder': 'Manzilni kiriting',
  'addresses.default': 'Asosiy',
  'addresses.empty_title': 'Manzillar yo\'q',
  'addresses.empty_subtitle': 'Yetkazib berish uchun manzil qo\'shing',
  'addresses.pick_on_map': 'Xaritadan tanlash',
  'addresses.selected_address': 'Tanlangan manzil',
  'addresses.tap_map': 'Joyni belgilash uchun xaritani suring',
  'maps.my_location': 'Mening joylashuvim',
  'maps.locating': 'Joylashuv aniqlanmoqda...',
  'maps.move_hint': 'Pinni kerakli joyga qo\'ying',
  'maps.point_selected': 'Xaritadagi belgilangan joy',
  'maps.locate_failed': 'Joylashuvni aniqlab bo\'lmadi. Xaritani qo\'lda suring.',
  'maps.locate_denied': 'Joylashuvga ruxsat berilmagan. Brauzer sozlamalaridan ruxsat bering yoki xaritani qo\'lda suring.',
  'addresses.choose_label': 'Yorliq tanlang',
  'addresses.label_home': 'Uy',
  'addresses.label_work': 'Ish',
  'addresses.label_other': 'Boshqa',
  'addresses.custom_label': 'Yorliq nomi',
  'addresses.save_address': 'Manzilni saqlash',
  'addresses.set_default': 'Asosiy qilish',
  'addresses.copy': 'Manzildan nusxa olish',
  'addresses.copied': 'Manzil nusxalandi',
  'addresses.comment': 'Izoh',
  'addresses.comment_placeholder': 'Qo\'shimcha ma\'lumot (kirish, qavat, eshik...)',
  'addresses.delete': 'O\'chirish',
  'addresses.delete_confirm_title': 'Manzilni o\'chirasizmi?',
  'addresses.delete_yes': 'Ha, o\'chirish',
  'addresses.map_unavailable': 'Xarita yuklanmadi. Internetni tekshiring va qayta urinib ko\'ring.',

  // Coupons
  'coupons.title': 'Kuponlar',
  'coupons.desc_first_order': 'Birinchi buyurtmaga chegirma',
  'coupons.desc_dairy': 'Sut mahsulotlariga chegirma',
  'coupons.desc_free_delivery': 'Bepul yetkazib berish',
  'coupons.expired': 'Muddati o\'tgan',
  'coupons.valid_until': 'Amal qilish:',
  'coupons.have_code': 'Kupon kodingiz bormi?',
  'coupons.enter_code': 'Kodni kiriting',
  'coupons.apply': 'Qo\'llash',
  'coupons.invalid': 'Kupon yaroqsiz',
  'coupons.saves': 'Tejash:',

  // Referral
  'referral.title': 'Mening referal kodim',
  'referral.subtitle': 'Do\'stlaringizga ulashing va bonus oling',
  'referral.copy': 'Nusxalash',
  'referral.copied': 'Nusxalandi!',

  // Referral rewards
  'referral.page_title': 'Referal va sovg\'alar',
  'referral.your_code': 'Sizning referal kodingiz',
  'referral.friend_gets': 'Do\'stingiz nima oladi',
  'referral.invited': 'Taklif qilingan',
  'referral.earned': 'Yutilgan',
  'referral.share': 'Havolani ulashish',
  'referral.my_rewards': 'Mening sovg\'alarim',
  'referral.reward_coupon': 'Chegirma kuponi',
  'referral.reward_free_delivery': 'Bepul yetkazib berish',
  'referral.reward_bonus': 'Bonus mahsulot',
  'referral.deliveries_remaining': '{count} ta bepul yetkazib berish qoldi',
  'referral.invited_friends': 'Taklif qilingan do\'stlar',
  'referral.rewarded': 'Mukofotlangan',
  'referral.pending': 'Kutilmoqda',
  'referral.apply_title': 'Referal kodini kiritish',
  'referral.apply_placeholder': 'Kodni kiriting',
  'referral.apply_success': 'Referal kod qo\'llanildi!',
  'referral.apply_error': 'Kod noto\'g\'ri',

  // Currency
  'currency': 'so\'m',

  // Common
  'common.go_catalog': 'Katalogga',
  'common.back': 'Orqaga',
  'common.yes': 'Ha',
  'common.no': 'Yo\'q',
  'common.loading': 'Yuklanmoqda...',
  'common.session_expired': 'Sessiya muddati tugadi. Qaytadan kiring.',
  'common.error_generic': 'Xatolik yuz berdi. Keyinroq urinib ko\'ring.',
  'common.clear': 'Tozalash',
  'common.done': 'Tayyor',

  // Checkout extras
  // Product
  'product.left_in_stock': 'Faqat {count} ta qoldi',
  'product.out_of_stock': 'Tugagan',
  'product.in_stock': 'Mavjud',
  'product.piece': 'dona',
  'product.description': 'Tavsif',
  'product.step': 'Qadam',
  'product.min': 'Minimal',
  'product.reviews': 'Sharhlar',
  'product.write_review': 'Sharh yozish',
  'product.review_placeholder': 'Fikringizni yozing...',
  'product.submit_review': 'Yuborish',
  'product.review_submitted': 'Sharh yuborildi!',
  'product.no_reviews': 'Hali sharhlar yo\'q',
  'product.login_to_review': 'Sharh yozish uchun kiring',
  'product.enter_quantity': 'Miqdorni kiriting',
  'product.by_qty': 'Miqdor',
  'product.by_sum': 'Summa bo\'yicha',
  'product.enter_sum': 'Summani kiriting',
  'cart.in_cart': 'savatchada',

  // Verify
  'verify.title': 'Telefon tasdiqlash',
  'verify.subtitle': 'Buyurtma berish uchun telefoningizni tasdiqlang',
  'verify.banner_title': 'Telefon tasdiqlanmagan',
  'verify.banner_subtitle': 'SMS orqali tasdiqlang',
  'verify.verify_now': 'Tasdiqlash',
  'verify.will_receive_sms': 'Ushbu raqamga SMS kod yuboriladi',
  'verify.send_sms': 'SMS yuborish',

  // Register extras
  'register.resend_in': '{sec} soniyadan keyin qayta yuborish',

  'checkout.note': 'Izoh',
  'checkout.note_placeholder': 'Buyurtma uchun qo\'shimcha izoh...',
  'checkout.select_address': 'Manzilni tanlang',
  'checkout.save_address_title': 'Manzilni saqlaymizmi?',
  'checkout.save_address_yes': 'Ha, saqlash',
  'checkout.save_address_no': 'Yo\'q, kerak emas',
  'orders.cancel': 'Bekor qilish',
  'orders.status_label.pending': 'Kutilmoqda',
  'orders.status_label.confirmed': 'Tasdiqlandi',
  'orders.status_label.completed': 'Yakunlandi',
  'orders.status_label.cancelled': 'Bekor qilindi',

  // ── Brand
  'brand.name': 'Bazar Market',
  'brand.bazar': 'Bazar',
  'brand.market': 'Market',
  'brand.go': 'Go',
  'brand.tagline': 'Mahalliy bozor — kunduzi',

  // ── Editorial: Home
  'ed.todays_edition': 'Bugungi nashr',
  'ed.fresh_l1': 'Bozordan',
  'ed.fresh_l2_italic': 'yangi mahsulotlar',
  'ed.this_weeks': 'Shu haftaning',
  'ed.featured_italic': 'tanlovi',
  'ed.featured_goods': 'mahsulotlari',
  'ed.departments': 'Bo\'limlar',
  'ed.aisles_l1': 'Saralangan',
  'ed.aisles_italic': 'bo\'limlar',
  'ed.view_all': 'Hammasi',
  'ed.editors_pick': 'Tahririyat tanlovi',
  'ed.editors_eyebrow': 'Ulashishga arziydigan ta\'m',
  'ed.editors_desc': 'Mutaxassislarimiz tomonidan ushbu haftaga maxsus tanlangan. Sifatga ishonchingiz komil bo\'lsin — biz bilan tezkor yetkazib beriladi.',
  'ed.this_week': 'Shu hafta',
  'ed.discover': 'Ko\'rish',
  'ed.collection': 'To\'plam',
  'ed.popular_italic': 'Ommabop',
  'ed.this_season': 'shu mavsumda',
  'ed.note_market': 'Bozordan eslatma',
  'ed.quote': 'Biz har kungi mahsulotlar ham noyob narsalar kabi e\'tibor talab qiladi deb ishonamiz.',
  'ed.curators': '— Tahririyat',
  'ed.department': 'Bo\'lim',
  'ed.fin': 'Tamom',
  'ed.signoff': 'Bugungi nashr shu yerda yakunlandi.',
  'ed.see_tomorrow': 'Ertaga ko\'rishguncha.',
  'ed.marquee': '100 000 so\'mdan oshganda bepul yetkazish · Ertalab terilgan mahsulotlar · Mahalliy tanlov · O\'rtacha 25 daqiqa · Naqd yoki karta · 24 soat ichida qaytarish',

  // ── Editorial: Cart
  'ed.your_basket_pre': 'Sizning',
  'ed.basket_italic': 'savatchangiz',
  'ed.continue_shopping': 'Xaridni davom ettirish',
  'ed.clear_all': 'Hammasini tozalash',
  'ed.basket_awaits': 'Savatcha sizni kutmoqda',
  'ed.start_selection': 'Bozorimizdan tanlashni boshlang.',
  'ed.browse_market': 'Bozorni ko\'rish',
  'ed.worth_adding': 'Qo\'shishga arziydi',
  'ed.little_extra': 'Ozgina qo\'shimcha',
  'ed.remove': 'O\'chirish',
  'ed.proceed': 'Davom etish',
  'ed.total_short': 'Jami',

  // ── Editorial: Checkout
  'ed.back_to_basket': 'Savatchaga qaytish',
  'ed.checkout_word': 'Buyurtma',
  'ed.place_order_pre': 'Buyurtma',
  'ed.place_order_italic': 'berish',
  'ed.delivery_to': 'Yetkazib berish manzili',
  'ed.contact_arrival': 'Aloqa va yetkazib berish',
  'ed.settle_arrival': 'Kelganda to\'lash',
  'ed.step_n_of': 'Bosqich',

  // ── Editorial: Product Detail
  'ed.back_market': 'Bozorga qaytish',
  'ed.about_product': 'Mahsulot haqida',
  'ed.from_market': 'Bozordan',
  'ed.price_label': 'Narx',
  'ed.per_label': 'Bir',
  'ed.kg_word': 'kilogramm',
  'ed.liter_word': 'litr',
  'ed.piece_word': 'dona',
  'ed.increment': 'Qadam',
  'ed.minimum': 'Minimum',
  'ed.add_to_basket': 'Savatchaga qo\'shish',
  'ed.in_basket': 'Savatchangizda',
  'ed.available': 'mavjud',
  'ed.quantity_word': 'Miqdor',

  // ── Editorial: Categories / Search
  'ed.browse': 'Ko\'rish',
  'ed.all_dep_pre': 'Barcha',
  'ed.all_dep_italic': 'bo\'limlar',
  'ed.subcategory': 'Pastki bo\'lim',
  'ed.in_category': 'da',
  'ed.back_to_dep': 'Bo\'limlarga qaytish',
  'ed.empty_shelves': 'Bo\'sh javonlar',
  'ed.try_aisle': 'Boshqa bo\'limni sinab ko\'ring',
  'ed.search_label': 'Qidiruv',
  'ed.search_market': 'Bozordan qidiring',
  'ed.discover_section': 'Topish',

  // ── Editorial: Favorites / Orders
  'ed.saved': 'Saqlangan',
  'ed.your_fav_pre': 'Sizning',
  'ed.fav_italic': 'sevimlilaringiz',
  'ed.archive': 'Arxiv',
  'ed.your_orders_pre': 'Sizning',
  'ed.orders_italic': 'buyurtmalaringiz',

  // ── Editorial: Profile & sub-views
  'ed.member': 'A\'zo',
  'ed.guest': 'Mehmon',
  'ed.account_section': 'Hisob',
  'ed.general_section': 'Umumiy',
  'ed.your_profile_pre': 'Sizning',
  'ed.profile_italic': 'profilingiz',
  'ed.your_settings_pre': 'Sizning',
  'ed.settings_italic': 'sozlamalaringiz',
  'ed.preferences': 'Afzalliklar',
  'ed.your_coupons_pre': 'Sizning',
  'ed.coupons_italic': 'kuponlaringiz',
  'ed.discounts_label': 'Chegirmalar',
  'ed.your_methods_pre': 'Sizning to\'lov',
  'ed.methods_italic': 'usullaringiz',
  'ed.payment_section': 'To\'lov',
  'ed.your_addr_pre': 'Sizning',
  'ed.addr_italic': 'manzillaringiz',
  'ed.delivery_label': 'Yetkazib berish',
  'ed.add_new_short': '+ Yangi',
  'ed.default_word': 'Asosiy',
  'ed.help_q_pre': 'Sizga qanday',
  'ed.help_q_italic': 'yordam beraylik',
  'ed.assistance': 'Yordam',
  'ed.hours_label': 'Ish vaqti',
  'ed.refer_pre': 'Do\'stga',
  'ed.refer_italic': 'havola',
  'ed.invite_earn': 'Taklif qil — daromad ol',

  // ── Editorial: Auth
  'ed.welcome_label': 'Xush kelibsiz',
  'ed.step_into_pre': 'Bozorga',
  'ed.step_into_italic': 'qadam qo\'ying',
  'ed.back_word': 'Orqaga',
  'ed.profile_word': 'Profil',
  'ed.join_label': 'Qo\'shilish',
  'ed.verify_label': 'Tasdiqlash',
  'ed.become_pre': 'A\'zo',
  'ed.become_italic': 'bo\'ling',
  'ed.last_pre': 'Oxirgi',
  'ed.last_italic': 'qadam',

  // ── Editorial: Common
  'ed.notice': 'Eslatma',
  'ed.confirm_word': 'Tasdiqlash',
  'ed.irreversible': 'Qaytarib bo\'lmaydi',
  'ed.empty_n': 'Bo\'sh',
  'ed.member_v1': 'Vol. 01 · № 01',
}
