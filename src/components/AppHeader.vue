  ...
  >
      ...
      <div class="flex items-center gap-2 cursor-pointer btn-press" @click="showDropdown = !showDropdown">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-0.5">
            ...
            <svg width="14" height="14" class="flex-shrink-0 transition-transform" :style="{ color: 'var(--text-secondary)', transform: showDropdown ? 'rotate(180deg)' : '' }" fill="currentColor" viewBox="0 0 24 24"><path d="M7 10l5 5 5-5z"/></svg>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="relative">
      <div class="flex items-center rounded-xl px-3 py-2.5 gap-2" style="background: var(--surface-secondary)">
        ...
        />
        <button v-if="searchQuery" @click="searchQuery = ''; showSearchResults = false; searchResults = []" class="btn-press p-0.5">
      ...
      <div v-if="showSearchResults && searchResults.length" class="absolute left-0 right-0 top-full mt-1 rounded-xl overflow-hidden z-50" style="background: var(--surface); box-shadow: 0 8px 32px var(--shadow-lg); max-height: 300px; overflow-y: auto;">
        <div v-for="product in searchResults.slice(0, 8)" :key="product.id"
          @click="goToProduct(product)"
          class="flex items-center gap-3 px-3 py-2.5 btn-press border-b" :style="{ borderColor: 'var(--border)' }">
          <img v-if="product.image" :src="product.image" class="w-10 h-10 rounded-lg object-contain flex-shrink-0" style="background: var(--surface-secondary)" />
          <div v-else class="w-10 h-10 rounded-lg flex-shrink-0" style="background: var(--surface-secondary)"></div>
          <div class="flex-1 min-w-0">
            <p class="text-xs font-bold truncate" style="color: var(--text-primary)">{{ getLocalizedName(product.name) }}</p>
            <p class="text-[10px] font-semibold" style="color: var(--text-secondary)">{{ product.price ? product.price.toLocaleString() + " so'm" : '' }}</p>
          </div>
        </div>
      </div>
    </div>
