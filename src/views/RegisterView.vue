...
async function handleSendCode() {
  ...
  if (!password.value || password.value.length < 4) { error.value = t('login.password_short'); return }
  error.value = ''
...
async function handleVerify() {
  ...
  if (result.success) {
    ...
    navigate('home')
  } else { error.value = t('register.wrong_code') }
}

function resendCode() { codeInputs.value = ['', '', '', '', '', '']; code.value = ''; sendCode(phone.value) }
</script>
    ...
    <div class="flex-1 px-4 -mt-4">
      <div v-if="step === 1">
        <div class="rounded-2xl p-5" style="background: var(--surface); box-shadow: 0 4px 24px var(--shadow-lg)">
            ...
            <div>
              ...
              <div class="flex items-center rounded-xl px-4 py-3 gap-3" style="background: var(--surface-secondary)">
                <svg width="18" height="18" class="flex-shrink-0" style="color: var(--text-tertiary)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" stroke-width="2" stroke-linecap="round"/><circle cx="12" cy="7" r="4" stroke-width="2"/>
                </svg>
          ...
          <button @click="handleSendCode" :disabled="isLoading"
            ...
            :class="{ 'opacity-60': isLoading }" style="box-shadow: 0 6px 24px var(--primary-glow)">
        ...
        <button @click="navigate('login')" class="w-full py-4 rounded-2xl font-black text-sm btn-press"
      ...
      <div v-if="step === 2">
        ...
        <button @click="handleVerify" :disabled="isLoading || code.length !== 6"
          ...
          :class="{ 'opacity-60': isLoading || code.length !== 6 }" style="box-shadow: 0 6px 24px var(--primary-glow)">
