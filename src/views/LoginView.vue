  ...
  <div class="min-h-screen flex flex-col" style="background: var(--bg-app)">
    <div class="relative pt-12 pb-8 flex flex-col items-center" style="background: linear-gradient(180deg, var(--primary) 0%, var(--primary-dark) 100%); border-radius: 0 0 32px 32px;">
      <div class="w-20 h-20 rounded-3xl bg-white/20 flex items-center justify-center mb-4 backdrop-blur-sm" style="box-shadow: 0 8px 32px rgba(0,0,0,0.15)">
        <img src="/logo.png" alt="Bazar Market" class="w-14 h-14 object-contain" />
      </div>
    ...
    <div class="flex-1 px-4 -mt-4">
        ...
        <div class="flex flex-col gap-3.5">
          ...
          <div>
            ...
            <div class="flex items-center rounded-xl px-4 py-3 gap-3" style="background: var(--surface-secondary)">
              ...
              <input v-model="password" :type="showPassword ? 'text' : 'password'" :placeholder="t('login.password_placeholder')" class="bg-transparent outline-none text-sm font-bold flex-1 min-w-0" style="color: var(--text-primary)" @keyup.enter="handleLogin" />
      ...
      <button @click="navigate('register')" class="w-full py-4 rounded-2xl font-black text-sm btn-press" style="background: var(--surface); color: var(--text-primary); box-shadow: 0 2px 12px var(--shadow)">{{ t('login.no_account') }}</button>
