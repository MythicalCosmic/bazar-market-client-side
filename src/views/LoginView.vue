...
<template>
  <div class="min-h-screen flex flex-col" style="background: var(--bg-app)">
    ...
    <div class="px-5 pt-6 pb-3 flex items-center justify-between">
      <button @click="step === 2 ? backToPhone() : navigate('home')" class="flex items-center gap-2 btn-press">
    ...
    <div class="px-5 pt-4 pb-6">
      <div class="rule mb-3">
        <span class="eyebrow">{{ t('ed.welcome_label') }}</span>
      </div>
      <h1 class="display text-[40px]" style="color: var(--text-primary); line-height: 0.95;">
        {{ t('ed.step_into_pre') }}<br/>
        <span class="serif-italic" style="color: var(--terracotta);">{{ t('ed.step_into_italic') }}</span>.
      </h1>
    </div>
    ...
    <div class="flex-1 px-5">
      ...
      <div v-if="step === 1">
        <div class="flex items-center gap-2 mb-3">
          ...
          <p class="eyebrow-sm">{{ t('login.title') }}</p>
        </div>
        <p class="serif-italic text-[14px] mb-5" style="color: var(--text-secondary)">{{ t('login.phone_subtitle') }}</p>
        ...
        <p v-if="error" class="serif-italic text-[13px] mt-3 text-center" style="color: var(--bordeaux)">{{ error }}</p>

        <button @click="handleSendCode" :disabled="isLoading" type="button"
          class="primary-btn btn-press mt-6"
          :class="{ 'opacity-50': isLoading }">
          <span class="eyebrow-sm" style="color: var(--cream)">{{ isLoading ? t('common.loading') : t('login.send_code') }}</span>
        ...
        <div class="rule-center my-7">
          <span class="eyebrow-sm" style="color: var(--text-tertiary)">{{ t('login.or') }}</span>
        </div>

        <button @click="navigate('register')" type="button" class="ghost-btn btn-press">
          <span class="eyebrow-sm">{{ t('login.no_account') }}</span>
        </button>
      ...
      <div v-else>
        <div class="flex items-center gap-2 mb-3">
          ...
          <p class="eyebrow-sm">{{ t('register.verify_title') }}</p>
        </div>
        ...
        <button @click="backToPhone" type="button" class="text-btn btn-press mt-4">
          <span class="serif-italic text-[13.5px]" style="color: var(--text-tertiary)">{{ t('login.use_different_phone') }}</span>
        </button>
      </div>
    </div>
  </div>
